...
object StatsUniqueClass {
    ...
    def analysisLog(_lastDays:Int, _year:String, _month:String, _day:String, sc:org.apache.spark.SparkContext, spark:org.apache.spark.sql.SparkSession) {
        ...
        val typeName = "class" + lastDays + "d"
        val typeNameND = "class" + lastDays + "d-nodev"
        ...
        // (class_id, resouce_type, role, country, device, [unique_cnt, data, lang, grade])
        val compactLogsRDD = activitylogsRDD.map { x =>
            if( langIdx < 0 )   {
                if( deviceIdx > 0 && !x.isNullAt(deviceIdx) )   {
                    val device = x.getAs[String](deviceIdx) + ""
                    ((x.getAs[String](targetIdIdx), x.getAs[String](roleIdx), device.replace(".event", ""), x.getAs[String](countryIdx), "_null"), 1)
                }
                else if( tagIdx > 0 && !x.isNullAt(tagIdx) )    {
                    val tag = x.getAs[String](tagIdx) + ""
                    ((x.getAs[String](targetIdIdx), x.getAs[String](roleIdx), tag.replace(".event", ""), x.getAs[String](countryIdx), "_null"), 1)
                }
                else    {
                    ((x.getAs[String](targetIdIdx), x.getAs[String](roleIdx), "_null", x.getAs[String](countryIdx), "_null"), 1)
                }
            }
            else    {
                if( deviceIdx > 0 && !x.isNullAt(deviceIdx) )   {
                    val device = x.getAs[String](deviceIdx) + ""
                    ((x.getAs[String](targetIdIdx), x.getAs[String](roleIdx), device.replace(".event", ""), x.getAs[String](countryIdx), x.getAs[String](langIdx)), 1)
                }
                else if( tagIdx > 0 && !x.isNullAt(tagIdx) )    {
                    val tag = x.getAs[String](tagIdx) + ""
                    ((x.getAs[String](targetIdIdx), x.getAs[String](roleIdx), tag.replace(".event", ""), x.getAs[String](countryIdx), x.getAs[String](langIdx)), 1)
                }
                else    {
                    ((x.getAs[String](targetIdIdx), x.getAs[String](roleIdx), "_null", x.getAs[String](countryIdx), x.getAs[String](langIdx)), 1)
                }
            }
        }
        ...
        val uniqueLogsDS = compactLogsRDD.reduceByKey(_ + _).map { log =>
            UniqueStats(log._1._1,log._1._2, log._1._3, log._1._4, log._2, todayDir, log._1._5, 0, timeStamp.toString, indexName, typeName)
        }
        .toDS
        ...
        uniqueLogsDS.coalesce(1).write.option("compression","none").parquet(s"$GS_OUTPUT_BUCKET/$indexName/$typeName/$todayDir")
        uniqueLogsDS2.coalesce(1).write.option("compression","none").parquet(s"$GS_OUTPUT_BUCKET/$indexName/$typeNameND/$todayDir")

        // println("Outputs:")
        // println(s"$GS_OUTPUT_BUCKET/$indexName/$typeName/$todayDir")
        // println(s"$GS_OUTPUT_BUCKET/$indexName/$typeNameND/$todayDir")
    } // end def analysisLog()
